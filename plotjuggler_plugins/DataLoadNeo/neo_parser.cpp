...
uint16_t extract_uint16(const std::deque<uint8_t>& data, uint8_t idx)
{
  uint16_t value = 0;
...
uint32_t extract_uint32(const std::deque<uint8_t>& data, uint8_t idx)
{
  uint32_t value = 0;
...
bool NeoParser::parse(const std::string& fileName)
{
  bool ok = false;
...
inline void NeoParser::addData(data_id id, uint32_t ts, double val)
{
  _data[id].push_back({ts-_timestamp, val});
}

void NeoParser::addInfo(const std::string& key, const std::string& val)
{
...
bool NeoParser::parseNeo(std::ifstream& ifs)
{
  ...
  static constexpr uint8_t neo_log_crc            = 14;       // CRC index
  static constexpr uint8_t neo_log_timestamp      = 4;        // TIMESTAMP index
  static constexpr uint8_t neo_log_distance       = 9;        // DATA index (distance in mm)
  static constexpr double neo_log_data_lsb        = 1.0e-3;   // DATA lsb (mm to m)
    ...
    do {
      ...
      if (start != buffer.end()) {
        ...
        if (buffer.size() >= neo_log_size) {
          ...
          if (extract_uint16(buffer, neo_log_crc) == crc16(buffer,neo_log_payload)) {
            ...
            do {
              buffer.pop_front();
              ++n;
            } while (!buffer.empty() && (n < neo_log_size));
          } else {
