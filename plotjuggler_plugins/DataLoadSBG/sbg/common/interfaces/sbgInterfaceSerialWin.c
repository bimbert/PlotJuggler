...
SbgErrorCode sbgInterfaceSerialCreate(SbgInterface *pHandle, const char *deviceName, uint32 baudRate)
{
    ...
    if (sscanf_s(deviceName, "COM%i", &deviceNum) == 1)
    {
        ...
        mbstowcs(wcomPortPath, comPortPath, strlen(comPortPath)+1);

        //
        // Init the com port
        //
        hSerialDevice = CreateFile(wcomPortPath, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
